...
import Write from "./Write5";
import Board from "./Board5"; // Add import for Board component
...
class BoardList5 extends Component<IProps, IState> {
  ...
  handleModify = (checkList: Array<string>) => {
    ...
    const selectedBoard = this.state.boardList.find(
      (board: any) => board.BOARD_ID5 === parseInt(checkList[0])
    );
  ...
  getList = () => {
    Axios.get("https://220.149.236.48:3306/list5", {})
      .then((res) => {
        const { data } = res;
        this.setState({
          boardList: data,
  ...
  handleDelete = () => {
    ...
    Axios.post("https://220.149.236.48:3306/delete5", {
          ...
          {boardList.map((v: any, index: number) => {
              return (
                <tr key={v.BOARD_ID}>
                  ...
                  <td>
                    <a
                      href={`220.149.236.48:3306/page3/${v.BOARD_ID5}`}
                      onClick={() => {
                        this.setState({
                          selectedBoardId: v.BOARD_ID5,
                          showWriteForm: false,
                          isModifyMode: false,
                        });
                      }}
                  ...
                  <td>{v.REGISTER_ID5}</td>
                  <td>{v.REGISTER_DATE5}</td>
                </tr>
              );
            })}
          </tbody>
