...
import Write from "./Write3";
import Board from "./Board3"; // Add import for Board component
...
class BoardList3 extends Component<IProps, IState> {
  ...
  handleModify = (checkList: Array<string>) => {
    ...
    const selectedBoard = this.state.boardList.find(
      (board: any) => board.BOARD_ID3 === parseInt(checkList[0])
    );
  ...
  getList = () => {
    Axios.get("http://localhost:8000/list3", {})
      .then((res) => {
        const { data } = res;
        this.setState({
          boardList: data,
  ...
  handleDelete = () => {
    ...
    Axios.post("http://localhost:8000/delete3", {
          ...
          {boardList.map((v: any, index: number) => {
              return (
                <tr key={v.BOARD_ID}>
                  ...
                  <td>
                    <a
                      href={`localhost:3000/page5/${v.BOARD_ID3}`}
                      onClick={() => {
                        this.setState({
                          selectedBoardId: v.BOARD_ID3,
                          showWriteForm: false,
                          isModifyMode: false,
                        });
                      }}
                  ...
                  <td>{v.REGISTER_ID3}</td>
                  <td>{v.REGISTER_DATE3}</td>
                </tr>
              );
            })}
          </tbody>
