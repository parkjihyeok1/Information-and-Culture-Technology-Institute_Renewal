...
import Write from "./Write2";
import Board from "./Board2"; // Add import for Board component
...
class BoardList2 extends Component<IProps, IState> {
  ...
  handleModify = (checkList: Array<string>) => {
    ...
    const selectedBoard = this.state.boardList.find(
      (board: any) => board.BOARD_ID2 === parseInt(checkList[0])
    );
  ...
  getList = () => {
    Axios.get("https://220.149.236.48:3306/list2", {})
      .then((res) => {
        const { data } = res;
        this.setState({
          boardList: data,
  ...
  handleDelete = () => {
    ...
    Axios.post("https://220.149.236.48:3306/delete2", {
          ...
          {boardList.map((v: any, index: number) => {
              return (
                <tr key={v.BOARD_ID}>
                  ...
                  <td>
                    <a
                      href={`220.149.236.48:3306/page4/${v.BOARD_ID2}`}
                      onClick={() => {
                        this.setState({
                          selectedBoardId: v.BOARD_ID2,
                          showWriteForm: false,
                          isModifyMode: false,
                        });
                      }}
                  ...
                  <td>{v.REGISTER_ID2}</td>
                  <td>{v.REGISTER_DATE2}</td>
                </tr>
              );
            })}
          </tbody>
