...
class BoardList extends Component<IProps, IState> {
  ...
  handleModify = (checkList: Array<string>) => {
    ...
    const selectedBoard = this.state.boardList.find(
      (board: any) => board.BOARD_ID === parseInt(checkList[0])
    );
  ...
  getList = () => {
    Axios.get("https://220.149.236.48:3306/list", {})
      .then((res) => {
        const { data } = res;
        this.setState({
          boardList: data,
  ...
  handleDelete = () => {
    ...
    Axios.post("https://220.149.236.48:3306/delete", {
            ...
            {boardList.map((v: any, index: number) => {
              return (
                <tr key={v.BOARD_ID}>
                  ...
                  <td>
                    <a
                      href={`220.149.236.48:3306/page3/${v.BOARD_ID}`}
                      onClick={() => {
                        this.setState({
                          selectedBoardId: v.BOARD_ID,
                          showWriteForm: false,
                          isModifyMode: false,
                        });
                      }}
